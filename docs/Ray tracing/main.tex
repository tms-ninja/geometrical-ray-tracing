...
The old method calls trigonometric functions including \texttt{sin()}, \texttt{cos()} and \texttt{atan2()}. If we were working in three dimensions we could write,
\begin{equation}
...
Now consider when the discriminant $1 - \gamma^2$ is less than zero. occurs for $\gamma > 1$ and $\gamma < -1$. Taking the magnitude of Eq.~\ref{eq:refraction_cross_prod}, $\gamma$ can be written in terms of the angle the incident ray makes to the normal of the plane as $\gamma = \frac{n_\mrm{i}}{n_\mrm{f}} \sin \theta$. We therefore do not have a real solution for $\vec{v}_\mrm{f}$ when,
\begin{align}
...
To distinguish which of the solutions is the one we are after, we note the sign of $\vec{v} \cdot \vec{\hat{D}}$ and $\vec{v} \cdot \vec{\hat{n}}$ must be unchanged after refraction.
